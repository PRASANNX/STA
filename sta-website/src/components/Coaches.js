...
export default function Coaches() {
  ...
  return (
    <section id="coaches" className="py-[72px] lg:py-[120px] px-6 lg:px-12 bg-background-2 reveal-item">
      ...
      <h2 className="font-heading text-[clamp(48px,6vw,72px)] leading-none tracking-[1px] mb-12 text-foreground">
        The Coaches<br />Behind Your<br /><span className="text-accent">Game</span>
      </h2>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {coaches.map((c, i) => (
          <div key={i} className="bg-surface border border-[rgba(200,232,53,0.15)] rounded-2xl p-7 relative transition-transform hover:-translate-y-1 hover:border-[rgba(200,232,53,0.4)]">
            {c.badge && (
              <span className="absolute top-6 right-6 bg-accent text-background px-3 py-1 rounded text-[10px] font-bold tracking-[1.5px] uppercase">
                {c.badge}
              </span>
            )}
            <div className="w-16 h-16 rounded-full bg-background border border-[rgba(200,232,53,0.3)] flex items-center justify-center font-heading text-[24px] text-accent mb-6 shadow-[0_0_15px_rgba(200,232,53,0.1)]">
              {c.initials}
            </div>
            <div>
              <div className="text-[12px] font-semibold text-accent uppercase tracking-[1px] mb-1">{c.role}</div>
              <div className="font-heading text-[28px] tracking-[1px] text-foreground mb-3">{c.name}</div>
